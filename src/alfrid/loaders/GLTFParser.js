...
const _getBufferViewData = (gltfInfo, bin) =>
  new Promise((resolve) => {
    const { bufferViews } = gltfInfo;

    bufferViews.forEach((bufferViewInfo) => {
      const buffer = bin;
    ...
    meshes.forEach((mesh) => {
      ...
      primitives.forEach((primitiveInfo) => {
        ...
        if (primitiveInfo.indices != null) {
          const attributeArray = _getAccessorData(
            gltf,
...
const _getAccessorData = (gltf, accessorIdx, isIndices = false) => {
  ...
  if (quantizeExtension) {
    ...
    for (let i = 0; i < accessorInfo.count; i++) {
      for (let k = 0; k < size; k++) {
        decodedArr[i * size + k] =
          arr[i * size + k] * decodeScale[k] + decodeOffset[k];
      }
    }
