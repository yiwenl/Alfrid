...
class GLTool {
  ...
  init(mCanvas, mParameters = {}) {
    ...
    mParameters.useWebgl2 =
      mParameters.useWebgl2 === undefined ? false : mParameters.useWebgl2;

    let ctx;
    if (mParameters.useWebgl2) {
      ctx =
        this.canvas.getContext("experimental-webgl2", mParameters) ||
        this.canvas.getContext("webgl2", mParameters);

      if (!ctx) {
        ctx =
          this.canvas.getContext("webgl", mParameters) ||
          this.canvas.getContext("experimental-webgl", mParameters);
        this._useWebGL2 = false;
      } else {
